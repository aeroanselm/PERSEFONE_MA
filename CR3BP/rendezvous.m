...
m_mars = mu_mars/G;
mu_phobos = 0.0007112;
m_phobos = mu_phobos/G;y0 = [x0(5) x0(7)];
...
r2BP = 9000;
v2BP = sqrt(mu_mars/r2BP);
vsyn = OM*r2BP;
dv = (v2BP - vsyn)/DU*TU;

p = 2*pi*sqrt(r2BP^3/mu_mars)/TU;
syn_p = (1/p - 1/(2*pi))^(-1);

x0 = [(x1 + r2BP/DU) 0 0 0 dv 0 syn_p];
[tv,state]=ode113(@(t,x)dynCR3BPpert(t,x,mu,0),[0 100*x0(7)],x0,options);
xs = state(:,1:3)*DU;
vs = state(:,4:6)*DU/TU;
state_dro = [xs vs];
