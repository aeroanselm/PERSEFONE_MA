...
for i = 20
    ...
    t = 0.7*pi;
    [tv,state]=ode113(@(t,x)dynCR3BPpert(t,x,mu,0),[0 10*t],s0,options);
    tv = tv*TU;
%     save('tv.mat','tv');
    xs = state(:,1:3)*DU;
    vs = state(:,4:6)*DU/TU;
...
% [~,state]=ode113(@(t,x)dynCR3BPpert(t,x,mu,0),[0:0.001:2*pi],s0,options);
% xs = state(:,1:3)*DU;
% vs = state(:,4:6)*DU/TU;
% state_sc = [xs vs];
% T = t*TU/3600;
...
% p1 = plot3(state_sc(1:i,1),state_sc(1:i,2),state_sc(1:i,3));%,'Linewidth',1.5);
% 
% p2 = plot3(p_sc(1),p_sc(2),p_sc(3),'o');
% 
% 
% p_sc = [p_sc(1:3)/DU (p_sc(4:6) - dv_dro)/DU*TU];
% [~,state]=ode113(@(t,x)dynCR3BPpert(t,x,mu,0),[0:0.1:130*pi],p_sc,options);
% xs = state(:,1:3)*DU;
% vs = state(:,4:6)*DU/TU;
% state_sc = [xs vs];
% T = t*TU/3600;
% p3 = plot3(state_sc(:,1),state_sc(:,2),state_sc(:,3));%,'y','Linewidth',1.5);
% legend([p1 p2 p3], 'Location', 'southeast', 'Circular Orbit', 'Intersection point', 'DRO') 
